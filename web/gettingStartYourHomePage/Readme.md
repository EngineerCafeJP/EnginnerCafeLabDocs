...
CSSを用いるとHtmlで描画される画面の要素に様々な装飾を食わえることができます。
...
あるいは慣例的に`style.css`という名前で運用され、htmlファイルから読み込むこともできます。

(このときファイルパスとHP上でのパスが一致しているか初心者は悩むかも！)


「htmlの要素」、「id」、「クラス」を冒頭で指定することができます。
また、デバイスごとに適用するCSSを変えてレスポンシブ対応にすることや、CSS上で画面の大きさに合わせた演算を行うこともできます。
